...
from core.config import (
    PENALTY_LEFT, SUCCESS, PENALTY_SUCCESS_REQUIREMENT,
    PENALTY_GOAL_REWARD, PENALTY_WAIT_MIN, PENALTY_WAIT_MAX,
    PENALTY_MIN_GOALS, PENALTY_MAX_GOALS
)
...
async def play_penalty(user_id: int) -> dict:
    ...
    wait_time = random.randint(PENALTY_WAIT_MIN, PENALTY_WAIT_MAX)
    await asyncio.sleep(wait_time)

    success_goals = random.randint(PENALTY_MIN_GOALS, PENALTY_MAX_GOALS)
    reward = success_goals * PENALTY_GOAL_REWARD
...
async def check_penalty_access(user_id: int) -> bool:
    ...
    if user_data is None:
        return False
    return user_data[SUCCESS] >= PENALTY_SUCCESS_REQUIREMENT
